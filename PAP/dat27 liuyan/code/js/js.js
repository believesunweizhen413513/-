(function () {
    ...
    */
    ...
    username1.onblur = () => {
        ...
        if (user1) {//非空验证
            //    用户名仅支持英文、数字和下划线（还没做中英文转译，暂时不做）
            var reg = /^[A-Za-z0-9_]+$/;
            var isok = reg.test(user1);
            if (isok) {
                verifyUserNameMsg.innerHTML = '验证通过';
                verifyUserNameMsg.style.color = '#58bc458';
                ajax({
                    type: 'get',
                ...
                verifyUserNameMsg.style.color = 'red';
                verifyUserNameMsg.innerHTML = '验证不通过';
            }

        } else {
            verifyUserNameMsg.innerHTML = '请输入内容';
            verifyUserNameMsg.style.color = 'red';
        }
    ...
    //拿到状态：获取cookie的账号，拿到意味着是登陆中(显示退出面板，隐藏登陆和注册面板)；反正就是退出状态；
    ...
    function update() {
        ...
        if (uid) {
            //登陆中
            user.style.display = 'block';//显示退出面板，隐藏登陆和注册面板
            login.style.display = 'none';//登录面板
            reg.style.display = 'none';//注册面板
    ...
    */
    ...
    function create(data, type) {
        ...
        if (type == 'one') {//one是生成留言，list是获取数据库默认的内容
    ...
    /*
            ...
            */
    let ipage = 1;//用一个变量来存页数，方便到时候让加载更多时，加载下一页的内容，ipage++
    ...
    function init() {//初始化列表
        ajax({
            ...
            success: str => {
                ...
                // create(arr.data);让留言大于1页且在最后一页之间显示加载更多
