...
function ajax(opt) {
    ...
    Object.assign(defaultOpt, opt);//用的时候记得用defaultOpt
    //1.创建对象
    let xhr = new XMLHttpRequest();
    // 2.写好参数 open（）,这里要判断是那种方式传输
    if (defaultOpt.type.toLowerCase() == 'get') {//假如是get传输方式，那么数据直接拼接在地址后面
        // 判断是否有数据
        if (defaultOpt.data) {
            //有数据，把数据拼接在url
