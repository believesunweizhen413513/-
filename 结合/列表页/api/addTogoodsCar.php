...
// 查找购物车中是否存在此数据
$sql = "SELECT * FROM goodscar WHERE username='$username' and gid='$gid'";
$res = $conn->query($sql);
...
if ($res->num_rows == 1) { //若存在数据,则更新数量,
    ...
    $sql3 = "update goodscar set counts = '$newCounts' WHERE gid='$gid' AND username= '$username'";
    ...
    // 先从数据表中获取数据,再添加到购物表中
    $sql2 = "SELECT * FROM goodslist2 WHERE uid='$gid'";
    $res2 = $conn->query($sql2); //执行
    $data = $res2->fetch_all(MYSQLI_ASSOC); //结果集，数组
    $shopname = $data[0]['shopname']; //店名
    $title = $data[0]['title']; //商品名
    $imgurl = $data[0]['url']; //图片路径
    $price = $data[0]['price']; //单价
    $sql4 = "INSERT INTO goodscar(username,gid,shopname,price,title,imgurl,counts) VALUES ('$username','$gid','$shopname','$price','$title','$imgurl','$counts')";
